...
| `--inference.rtc.prefix_guidance`           | Condition chunks on the previous chunk's tail (default: true)                  |
| `--inference.queue_threshold`               | Backpressure threshold; trained RTC requires at least its maximum delay        |

`prefix_guidance=false` is a guided-mode ablation. Trained RTC requires its hard prefix and rejects this combination.
