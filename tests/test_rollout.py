...
def test_rtc_observation_packet_keeps_the_pre_dispatch_queue_snapshot():
    from lerobot.policies.rtc import ActionQueue
    from lerobot.policies.rtc.configuration_rtc import RTCConfig

    engine = _make_rtc_engine([])
    queue = ActionQueue(RTCConfig(execution_horizon=4))
    original = torch.arange(8, dtype=torch.float32).view(4, 2)
    processed = original + 100
    queue.merge(original, processed, real_delay=0, task="test")
    engine._action_queue = queue

    engine.notify_observation({"state": 0.0})
    engine.get_action(None)  # dispatch happens after publication on the control thread
...
def test_trained_rtc_rejects_disabling_its_hard_prefix():
    from lerobot.policies.rtc.configuration_rtc import RTCConfig

    with pytest.raises(ValueError, match="requires prefix_guidance=True"):
        RTCConfig(mode="trained", prefix_guidance=False)
