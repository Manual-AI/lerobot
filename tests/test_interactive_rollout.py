...
def test_rtc_engine_reset_discards_chunk_from_inflight_inference(caplog):
    ...
    with _running_rtc_engine() as (engine, policy):
        ...
        # reset invalidates the epoch immediately, then waits for the policy-owning
        # inference to leave its critical section before resetting stateful policy and
        # processor objects. Run it on the caller thread shape used by the controller,
        # but release the deliberately blocked policy from this test thread.
        reset_thread = Thread(target=engine.reset, name="RTCResetTest")
        reset_thread.start()
        assert _wait_for(lambda: engine._reset_epoch == 1)
